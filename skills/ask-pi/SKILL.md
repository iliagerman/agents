...
7. Do not set a thinking level for ordinary questions. Delegated code reviews default to `medium`; change it only when the user explicitly requests another level.
...
Reviews use `medium` thinking by default. If the user explicitly requests another level, set `PI_REVIEW_THINKING` for that invocation:
...
- selects `openai-codex/gpt-5.6-sol` with `--thinking medium` by default, or the explicitly requested `PI_REVIEW_THINKING` value
