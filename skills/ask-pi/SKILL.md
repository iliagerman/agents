---
name: ask-pi
description: Explains how to call pi non-interactively from a shell, script, CI job, or cron task using the `github-copilot/gpt-5.4` model. Use this whenever the user wants to pass a question from the CLI, automate pi, run a one-shot unattended prompt, capture pi output in a variable, or wrap pi in bash.
compatibility: Requires `pi` on PATH and an authenticated pi setup that can access `github-copilot/gpt-5.4`.
---
...
```bash
pi -p --no-session --model github-copilot/gpt-5.4 "<question>"
```
...
4. Use `--model github-copilot/gpt-5.4` exactly when the user asks for GPT-5.4.
...
```bash
pi -p --no-session --model github-copilot/gpt-5.4 "What ports are listening on this machine?"
```
...
```bash
ANSWER=$(pi -p --no-session --model github-copilot/gpt-5.4 "Write a conventional commit message for the current git diff")
printf '%s\n' "$ANSWER"
```
...
```bash
git diff --stat | pi -p --no-session --model github-copilot/gpt-5.4 "Summarize this diff briefly"
```

### Read from a file and ask a question

```bash
pi -p --no-session --model github-copilot/gpt-5.4 @README.md "Summarize the main setup steps"
```

## Wrapper script

If the user wants a reusable command, use the bundled helper script:

```bash
./scripts/ask-pi.sh "Explain what this shell script does"
```
...
- Log in once in pi and choose GitHub Copilot via `/login`
